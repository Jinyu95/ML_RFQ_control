from train_model import LoadModelandScalers
...
if __name__ == '__main__':
    ...
    dir = 'model_1020to0116_interval5s'
    model,  sc_x, sc_d, sc_i= LoadModelandScalers(dir)
    ...
    while True:
        ...
        if len(FreqHistory) >= start_step:
            trainu_con = np.zeros((start_step * 2, 13))
            trainx_con = np.zeros((start_step * 2, 1))
            for i in range(start_step):
                trainu_con[i, 0] = DelayedHistory[0][i]
                trainu_con[i, 1] = DelayedHistory[1][i]
                trainu_con[i, 2] = DelayedHistory[2][i]
                trainu_con[i, 3] = DelayedHistory[3][i]
                trainu_con[i, 4] = DelayedHistory[4][i]
                trainu_con[i, 5] = DelayedHistory[5][i]
                trainu_con[i, 6] = DelayedHistory[6][i]
                trainu_con[i, 7] = DelayedHistory[7][i]
                trainu_con[i, 8] = DelayedHistory[8][i]
                trainu_con[i, 9] = DelayedHistory[9][i]
                trainx_con[i, 0] = FreqHistory[i]
            ...
            u1_new, u2_new = run_newtons_method(u1, u2, trainx_con, trainu_con, model, sc_x, sc_d, sc_i)
            ...
            u1_new, u2_new = apply_constraint(u1_new, u2_new)
